...
export default function UploadCode() {
  ...
  const HandelTaskSubmit = async () => {
    ...
    if (selectuploadActivity.length === 0) {
    ...
    try {
      const response = await axios.post("/api/task", {
        ...
        task_content: selectuploadActivity,
        type: "Code",
        url: `${window.location.href.replace('/dashboard/upload-code', '')}/student?user_id=${user.id}&task_title=${encodeURIComponent(usertask.task_title)}&type=Code`
      });
  ...
  const copyToClipboard = async () => {
    try {
      await navigator.clipboard.writeText(`${window.location.href.replace('/dashboard/upload-code', '')}/student?user_id=${user.id}&task_title=${encodeURIComponent(usertask.task_title)}&type=Code`);
    ...
    <div>
      ...
      <div className="flex flex-col bg-white rounded-2xl mx-10 p-10 mb-20">
        ...
        <div className="flex mb-6 p-5 border-1 border-blue-200 rounded-2xl justify-center text-gray-500">
          {selectuploadActivity.length > 0 ? (
            <>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {selectuploadActivity.map((items, index) => (
                  <div
                    key={index}
                    className={
                  ...
                  >
                    ...
                    <div className="mt-4 m-2">
                      ...
                      <p className="text-gray-600" title={items.code_content.split(" ").slice(0, 30).join(" ")}>
            ...
            <h1>Select Code Activities To Create Task</h1>
          )}
        </div>
        <Button
          ...
          <div className="flex items-center space-x-2 mt-4">
            <div className="grid flex-1 gap-2">
              <Input
                ...
                value={`${window.location.href.replace('/dashboard/upload-code', '')}/student?user_id=${user.id}&task_title=${encodeURIComponent(usertask.task_title)}&type=Code`}
                readOnly
              />
            </div>
            <Button 
